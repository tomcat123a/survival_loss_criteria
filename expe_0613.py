...
def c_index_depre(pred, ytime, yevent):
	#yevent=torch.tensor(yevent)
    #yevent = yevent.view(-1,1).type(torch.float)   
    #pred=pred.view(-1)
...
def c_index(pred, ytime, yevent):
   ...
   # n_uncensored = yevent.sum(0)
    ...
    #pred_matrix = torch.zeros_like(ytime_matrix)
    pred=pred.view(-1)
    denomi=(ytime.expand(n_sample,n_sample).transpose(0,1)<ytime.expand(n_sample,n_sample))[uncensor_idx].sum()
    nume=(pred.expand(n_sample,n_sample).transpose(0,1)<pred.expand(n_sample,n_sample))[uncensor_idx].sum()
    concordance_index = nume.numpy()/denomi.numpy()
	###if gpu is being used
    if torch.cuda.is_available():
        concordance_index = concordance_index.cuda()
	###
...
def get_data_edge_lung(survpath,data_num,data_type='edge',train_rate=0.5,device='cpu'):
    ...
    if data_type=='edge' or data_type=='edge_weight':
        ...
        return data_list,y,censor,trainset,testset,x.shape[0],adjt,adjt
    else:
...
def train_c(train_loader, y_train, censor_train,mode,x_train=None,adj_train=None,device='gpu',model=None,optimizer=None,H_1=1):
    model.train()
...
def do_c(maxit, y_train, censor_train, y_test, censor_test,mode,train_loader,test_loader,x_train,x_test,adj_train,adj_test,optimizer,device,model=None):
    ...
    for epoch in range(1, maxit):
        temp_loss=train_c(train_loader, y_train, censor_train,mode,x_train,adj_train,device,model,optimizer)
        #print('training at epoch {} starts.'.format(epoch))
        if epoch %2==0:
            print('training_loss at epock{} = {}'.format(epoch,temp_loss.cpu().data.numpy().ravel()[0]))
...
def experiment(data_num,process_type,epoch,n_channels,jump_type,gnn_type,gnn_k,survpath):
    t0=time.time()
    ...
    #gnn_type=2-3GCONV,9-14GAT,4-5Cheb,7-8GatedGraph,6GraphConv,15SG,14ARMA #6,9,10,11,12 only edge
        ...
        model = Netfc(num_features,fc_node_list=[143,1]).type(torch.float32).to(device)
...
for i in range(10):
    ...
    t0=time.time()
    if process_type=='adj':
        cidx=experiment(data_num=0,process_type='adj',epoch=3,n_channels=4,jump_type='max',gnn_type=1,gnn_k=2\
                   ,survpath='C:/Users/Administrator/Desktop/deepgraphsurv/lung_cancer' \
                   if os.name=='nt' else '/home/yilun/remote_dir')
    else:
        cidx=experiment(data_num=3,process_type='edge',epoch=10,n_channels=20,jump_type='max',gnn_type=12,gnn_k=2\
               ,survpath='C:/Users/Administrator/Desktop/deepgraphsurv/lung_cancer' \
               if os.name=='nt' else '/home/yilun/remote_dir')
    t1=time.time()-t0
    c_idx.append(cidx)
...
pd.DataFrame({'c_idx':c_idx,'timing':t_consume}).to_csv('C:/Users/Administrator/Desktop/deepgraphsurv/result_dir/simu_result.csv' \
            if os.name=='nt' else '/home/yilun/result_dir/simu_result_fc.csv')
c_idx=[]
t_consume=[]
process_type='edge'
for i in range(10):
    ...
    t0=time.time()
    if process_type=='adj':
        cidx=experiment(data_num=5,process_type='adj',epoch=1000,n_channels=4,jump_type='max',gnn_type=1,gnn_k=2\
                   ,survpath='C:/Users/Administrator/Desktop/deepgraphsurv/lung_cancer' \
                   if os.name=='nt' else '/home/yilun/remote_dir')
    else:
        cidx=experiment(data_num=3,process_type='edge',epoch=1000,n_channels=20,jump_type='max',gnn_type=12,gnn_k=2\
               ,survpath='C:/Users/Administrator/Desktop/deepgraphsurv/lung_cancer' \
               if os.name=='nt' else '/home/yilun/remote_dir')
    t1=time.time()-t0
    c_idx.append(cidx)
    ...
    #'C:/Users/Administrator/Desktop/deepgraphsurv/lung_cancer'
pd.DataFrame({'c_idx':c_idx,'timing':t_consume}).to_csv('C:/Users/Administrator/Desktop/deepgraphsurv/result_dir/simu_result.csv' \
            if os.name=='nt' else '/home/yilun/result_dir/simu_result_gat.csv')
