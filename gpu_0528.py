...
def get_data_edge_lung(survpath,data_num,data_type='edge',train_rate=0.7):
...
class GNN(torch.nn.Module):
    def __init__(self,
        ...
        if gnn_type==0:
            ...
            self.conv2 = nn.Sequential(*[DenseSAGEConv(in_channels=hidden_channels, out_channels=out_channels, normalize=False)]*(self.k-1))
        
            #self.conv2 = nn.Sequential(*[DenseSAGEConv(in_channels=hidden_channels, out_channels=out_channels, normalize=False)]*(self.k-1))
        if gnn_type==1:
            self.conv1 = DenseSAGEConv(in_channels=self.in_channels, out_channels=out_channels, normalize=True)
            self.conv2 = nn.Sequential(*[DenseSAGEConv(in_channels=hidden_channels, out_channels=out_channels, normalize=True)]*(self.k-1))
        
        if gnn_type==2:
            self.conv1 = GCNConv(in_channels=1, out_channels=out_channels, cached=False)
            self.conv2 = nn.Sequential(*[GCNConv(in_channels=hidden_channels, out_channels=out_channels, cached=False)]*(self.k-1))
        if gnn_type==3:
            self.conv1 = GCNConv(in_channels=1, out_channels=out_channels,improved=True, cached=False)
            self.conv2 = nn.Sequential(*[GCNConv(in_channels=hidden_channels, out_channels=out_channels,improved=True, cached=False)]*(self.k-1))
        if gnn_type==4:
            self.conv1 = ChebConv(in_channels=1, out_channels=out_channels,K=2)
            self.conv2 = nn.Sequential(*[ChebConv(in_channels=hidden_channels, out_channels=out_channels,K=2)]*(self.k-1))
        if gnn_type==5:
            self.conv1 = ChebConv(in_channels=1, out_channels=out_channels,K=4)
            self.conv2 = nn.Sequential(*[ChebConv(in_channels=hidden_channels, out_channels=out_channels,K=4)]*(self.k-1))
        if gnn_type==6:
            self.conv1 = GraphConv(in_channels=1, out_channels=out_channels,aggr='add')
            self.conv2 = nn.Sequential(*[GraphConv(in_channels=hidden_channels, out_channels=out_channels,aggr='add')]*(self.k-1))
        if gnn_type==7:
        ...
        if gnn_type==9:
            ...
            self.conv2 =nn.Sequential(*[GATConv(in_channels=hidden_channels,out_channels=out_channels, heads=1, concat=True, negative_slope=0.2,dropout=0.6)]*(self.k-1))
        if gnn_type==10:
            self.conv1 =GATConv(in_channels=1,out_channels=out_channels, heads=6, concat=False, negative_slope=0.2,dropout=0.6)
            self.conv2 =nn.Sequential(*[GATConv(in_channels=hidden_channels,out_channels=out_channels, heads=6, concat=False, negative_slope=0.2,dropout=0.6)]*(self.k-1))
            
        if gnn_type==11:
            self.conv1 =GATConv(in_channels=1,out_channels=out_channels, heads=4, concat=True, negative_slope=0.2,dropout=0)
            self.conv2 =nn.Sequential(*[GATConv(in_channels=hidden_channels,out_channels=out_channels, heads=4, concat=True, negative_slope=0.2,dropout=0.6)]*(self.k-1))
        
        if gnn_type==12:
            self.conv1 =GATConv(in_channels=1,out_channels=out_channels, heads=4, concat=False, negative_slope=0.2,dropout=0.6)
            self.conv2 =nn.Sequential(*[GATConv(in_channels=hidden_channels,out_channels=out_channels, heads=4, concat=False, negative_slope=0.2,dropout=0.6)]*(self.k-1))
        ...
        if gnn_type==14:
            ...
            self.conv2 = nn.Sequential(*[ARMAConv(in_channels=hidden_channels, out_channels=out_channels, num_stacks=1, num_layers=1, 
                                  shared_weights=False, act=F.relu, dropout=0.5, bias=True)]*(self.k-1))
        if gnn_type==15:
            self.conv1 = SGConv(in_channels=1, out_channels=out_channels, K=2, cached=True, bias=True)
            self.conv2 = nn.Sequential(*[SGConv(in_channels=hidden_channels, out_channels=out_channels, K=2, cached=True, bias=True)]*(self.k-1))
        if gnn_type==16:
            self.conv1 = SGConv(in_channels=1, out_channels=out_channels, K=3, cached=True, bias=True)
            self.conv2 = nn.Sequential(*[SGConv(in_channels=hidden_channels, out_channels=out_channels, K=3, cached=True, bias=True)]*(self.k-1))
        if gnn_type==17:
        ...
        if gnn_type==19:
            ...
            self.conv2 =nn.Sequential(*[RGCNConv(in_channels=hidden_channels, out_channels=out_channels, num_relations=3, num_bases=2, bias=True)]*(self.k-1))
# =============================================================================
#         if gnn_type==20:
...
# =============================================================================
        if gnn_type==25:
            ...
            self.conv2 = nn.Sequential(*[SplineConv(in_channels=hidden_channels, out_channels=out_channels, dim=2, kernel_size=3, is_open_spline=True, 
        ...
        if gnn_type==26:
            self.conv1 = SplineConv(in_channels=1, out_channels=out_channels, dim=3, kernel_size=3, is_open_spline=False, 
                                    degree=1, norm=True, root_weight=True, bias=True)
            self.conv2 = nn.Sequential(*[SplineConv(in_channels=hidden_channels, out_channels=out_channels, dim=3, kernel_size=3, is_open_spline=False, 
                                    degree=1, norm=True, root_weight=True, bias=True)]*(self.k-1))
        if gnn_type==27:
            self.conv1 = SplineConv(in_channels=1, out_channels=out_channels, dim=3, kernel_size=6, is_open_spline=True, 
                                    degree=1, norm=True, root_weight=True, bias=True)
            self.conv2 = nn.Sequential(*[SplineConv(in_channels=hidden_channels, out_channels=out_channels, dim=3, kernel_size=6, is_open_spline=True, 
                                    degree=1, norm=True, root_weight=True, bias=True)]*(self.k-1))
        if gnn_type==28:
            ...
            self.conv2 = nn.Sequential(*[SplineConv(in_channels=hidden_channels, out_channels=out_channels, dim=3, kernel_size=3, is_open_spline=True, 
...
def train_c(train_loader, y_train, censor_train,mode,x_train=None,adj_train=None,device='gpu',H_1=1):
    model.train()
    if mode=='edge'or mode=='edge_weight':
        ...
        c_idx = c_index(output,torch.from_numpy(y_train),torch.from_numpy( censor_train))
        loss = neg_par_log_likelihood(output, torch.from_numpy(y_train), torch.from_numpy(censor_train)) + H_1*( link_loss + ent_loss)#event=0,censored
        loss.backward()
        optimizer.step()
...
def test(test_loader,y_test,censor_test,mode,x_test=None,adj_test=None,device='gpu'):
    model.eval()
    if mode=='edge'or mode=='edge_weight':
...
def do_c(maxit, y_train, censor_train, y_test, censor_test,mode,train_loader,test_loader,x_train,x_test,adj_train,adj_test,optimizer,device):
    
    for epoch in range(1, maxit):
        temp_loss,train_cidx=train_c(train_loader, y_train, censor_train,mode,x_train,adj_train,device)
    ...
    test_cidx = test(test_loader, y_test, censor_test,mode,x_test,adj_test,device)
    return test_cidx
#########################################################################
# =============================================================================
...
def experiment(data_num,process_type,epoch,n_channels,r_shrink,jump_type,gnn_type,gnn_k,survpath,sh_f,mod):
     
    
    if process_type!='adj':
        data_list,y,censor,trainset,testset,num_features,adj_train,adj_test = get_data_edge_lung(survpath,data_num,data_type=process_type)
    else:
        data_list,y,censor,trainset,testset,num_features,adj_train,adj_test = get_data_edge_lung(survpath,data_num,data_type=process_type)
    ...
    #gnn_type=2-3GCONV,9-14GAT,4-5Cheb,7-8GatedGraph,6GraphConv,15SG,14ARMA #6,9,10,11,12 only edge
        model = Net2(num_features,hidden_channels=n_channels,out_channels=n_channels,gnn_type=gnn_type,gnn_k=gnn_k,res=True,jump=jump_type).to(device)
    else:
        model = Net_diff_pool2(num_features,hidden_channels=n_channels,out_channels=n_channels,out_clusters=int(num_features*r_shrink),gnn_k=gnn_k,res=True,jump=jump_type,shrink_factor=sh_f).to(device)
...
experiment(5,'adj',3,3,0.5,'lstm',1,2,'C:/Users/Administrator/Desktop/deepgraphsurv/lung_cancer',0.3)
