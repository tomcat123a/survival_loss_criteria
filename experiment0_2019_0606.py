...
from torch_geometric.nn import  dense_diff_pool,GCNConv,ChebConv
...
def dense_diff_pool(x, adj, s, mask=None):
    ...
    torch.stack([torch.norm(adj - torch.matmul(s[i], s[i].transpose(0, 1)), p=2) for i in range(batch_size)]).sum(dim=0)
...
def get_data_edge_lung(survpath,data_num,data_type='edge',train_rate=0.1,device='cpu'):
...
def train_c(train_loader, y_train, censor_train,mode,x_train=None,adj_train=None,device='gpu',model=None,optimizer=None,H_1=1):
    ...
    if mode=='edge'or mode=='edge_weight':
        ...
        loss = neg_par_log_likelihood(output, y_train, censor_train) + H_1*( link_loss + ent_loss)#event=0,censored
        loss.backward()
        optimizer.step()
...
def do_c(maxit, y_train, censor_train, y_test, censor_test,mode,train_loader,test_loader,x_train,x_test,adj_train,adj_test,optimizer,device,model=None):
    ...
    for epoch in range(1, maxit):
        temp_loss,train_cidx=train_c(train_loader, y_train, censor_train,mode,x_train,adj_train,device,model,optimizer)
        print('training at epoch {} starts.'.format(epoch))
        if epoch %1==0:
            print('training_loss at epock{} = {},train_c_index = {}'.format(epoch,temp_loss.cpu().data.numpy().ravel()[0],train_cidx.cpu()))
...
def experiment(data_num,process_type,epoch,n_channels,jump_type,gnn_type,gnn_k,survpath):
    ...
    #gnn_type=2-3GCONV,9-14GAT,4-5Cheb,7-8GatedGraph,6GraphConv,15SG,14ARMA #6,9,10,11,12 only edge
        model = Net0(num_features,out_channels=n_channels,gnn_type=gnn_type,gnn_k=gnn_k,res=True,jump=jump_type).to(device)
    else:
        model = Netadj(num_features,out_channels=n_channels,out_clusters_list=[20,10],gnn_k=gnn_k,res=False,jump=jump_type).type(torch.float32).to(device)
...
for i in range(2):
    ...
    cidx=experiment(data_num=3,process_type='adj',epoch=3,n_channels=2,jump_type=None,gnn_type=1,gnn_k=2\
               ,survpath='C:/Users/Administrator/Desktop/deepgraphsurv/lung_cancer' \
...
pd.DataFrame({'c_idx':c_idx,'timing':t_consume}).to_csv('C:/Users/Administrator/Desktop/deepgraphsurv/result_dir/simu_result.csv' \
            if os.name=='nt' else '/home/yilun/result_dir/simu_result.csv')
