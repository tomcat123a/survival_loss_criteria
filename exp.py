...
def get_data_edge_lung(survpath,data_num,data_type='edge',train_rate=0.6):
...
def train_c(train_loader, y_train, censor_train,mode,x_train=None,adj_train=None,device):
    model.train()
...
def do_c(maxit, y_train, censor_train, y_test, censor_test,mode,train_loader,test_loader,x_train,x_test,adj_train,adj_test,optimizer):
...
#gnn_type=2-3GCONV,9-14GAT,4-5Cheb,7-8GatedGraph,6GraphConv,15SG,14ARMA #6,9,10,11,12 only edge
    model = Net0(num_features,hidden_channels=n_channels,out_channels=n_channels,gnn_type=gnn_type,gnn_k=gnn_k,res=True,jump=jump_type).to(device)
else:
    model = Net_diff_pool(num_features,hidden_channels=n_channels,out_channels=n_channels,out_clusters=num_features*r_shrink,gnn_k=gnn_k,res=True,jump=jump_type).to(device)

optimizer = torch.optim.Adam(model.parameters(), lr=0.001)

ret=do_c(epoch, y_train, censor_train, y_test, censor_test,process_type,train_loader,test_loader,x_train,x_test,adj_train,adj_test,optimizer)
